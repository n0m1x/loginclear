...
int main(int argc, char* argv[]) {
    ...
    if ((user_param.users_file == "") ^ (user_param.passwords_file == "")) {
        std::cerr << "Both -u and -p required" << std::endl;
        return EXIT_FAILURE;
    } else if (user_param.logins_file == "" && (user_param.users_file == "" || user_param.passwords_file == "")) {
        std::cerr << "No logins or users and passwords file specified (specify with -l or -u and -p)" << std::endl;
        return EXIT_FAILURE;
    } else if (user_param.logins_file != "" && (user_param.users_file != "" || user_param.passwords_file != "")) {
        std::cerr << "Cannot set both -l and -u or -p, only -l or -u and -p" << std::endl;
        return EXIT_FAILURE;
    } else if (user_param.request_file == "") {
        std::cerr << "No requests file (-r)" << std::endl;
        return EXIT_FAILURE;
    } else if (user_param.string_patterns.empty() && user_param.regex_patterns.empty()) {
        std::cerr << "No patterns defined (-n/-N)" << std::endl;
        return EXIT_FAILURE;
    } else if (user_param.domain == "") {
        std::cerr << "No domain or IP specified (-d)" << std::endl;
        return EXIT_FAILURE;
        ...
        std::cerr << "Connecting to ip-or-domain[:port] failed (-d)" << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (user_param.verbose) {
        ...
        std::cout << "logins_file: " << user_param.logins_file << std::endl;
    ...
    if (getpid() == main_pid) {
        ...
        if (user_param.logins_file != "") {
            total_combinations = count_lines(user_param.logins_file);
            if (total_combinations == -1) {
                std::cerr << "Error: Could not open the file " << user_param.logins_file << std::endl;
                return EXIT_FAILURE;
            }
            file1.open(user_param.logins_file);
            if (!file1.is_open()) {
                std::cerr << "Error: Could not open the file " << user_param.logins_file << std::endl;
                return EXIT_FAILURE;
            }
            buffer_in_parent = process_logins_file(file1, BUFFER_IN_IPC_SIZE * user_param.num_proc);
        } else {
        ...
        while (!childrenFinished) {
            ...
            if (buffer_in_parent.empty()) {
                if (user_param.logins_file != "" && !file1.eof()) {
                    buffer_in_parent = process_logins_file(file1, BUFFER_IN_IPC_SIZE * user_param.num_proc);
