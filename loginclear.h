...
struct ParamStruct {
    std::string logins_file;            // -l
    std::string request_file;           // -r
    std::string users_file;             // -u [optional]
    std::string passwords_file;         // -p [optional]
    std::string domain;                 // -d
...
void print_help() {
    ...
    std::cout << "loginclear { -l <logins_file> | ( -u <users_file> -p <passwords_file> ) } -r <request_file> -d <url> { -n <regexp> | -N <string> } [OPTIONS]" << std::endl;
    std::cout << std::endl;
    std::cout << "DESCRIPTION" << std::endl;
    std::cout << "Loginclear is a brute forcing tool for web logins." << std::endl;
    std::cout << std::endl;
    std::cout << "OPTIONS" << std::endl;
    std::cout << "  -l <file>     : Logins file (may be replaced by -u and -p)" << std::endl;
    std::cout << "  -u <file>     : Users file (optional)" << std::endl;
    std::cout << "  -p <file>     : Passwords file (optional)" << std::endl;
...
// Read login file to memory (return vector of pairs)
std::vector<std::pair<std::string, std::string>> process_logins_file(std::ifstream& file, int buffer_size) {
    // Vector to store logins
    std::vector<std::pair<std::string, std::string>> result;

    // Read each line from the file
    std::string line;
    int combinations = 0;
    while (combinations < buffer_size && std::getline(file, line)) {
        ...
        if (pos != std::string::npos) {
            ...
            result.emplace_back(std::make_pair(first_part, second_part));
            combinations++;
        }
    }
